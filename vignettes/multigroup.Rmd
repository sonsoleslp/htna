...
`permutation_htna()` provides the non-parametric significance test; pass the
...
Patterns now span all three actor types. The default is state-level, with
a `meta_schema` rollup column tagging each row with its type-level
template:
